...
**Audience (second display):** when `audience.enabled` is true in `configs/app.yml`, starting **Free Switch** also starts a **LiveKit publisher** that publishes **`broadcast_video` at `1920×1080`** (native VR line + optional 2D avatar PiP) plus **`narration`** (TTS PCM). Viewers use a local HTTP page on port **8080** (default). Wait for **`[MEDIA] connected`** in the GUI terminal. Higher resolution uses more uplink and encoder load; see **[src/miis_broadcast/audience/README.md](src/miis_broadcast/audience/README.md)** for Docker, firewall, flowcharts, and logs.
