...
The session uses a **single-slot `_frame_queue`** (`maxsize=1`, latest FRAME overwrites) so processing stays near real time without a deep backlog.
...
For a full narrative (20 fps default send / PREVIEW baseline, protocol field list), see the root
